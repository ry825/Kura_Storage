...
load_config() {
    ...
    local required=(
        ...
        KURASTORAGE_MEDIA_CLEANUP_RUN_LEASE_MINUTES
    ...
    ((KURASTORAGE_MEDIA_CLEANUP_MANUAL_POLL_SECONDS >= 1 && KURASTORAGE_MEDIA_CLEANUP_MANUAL_POLL_SECONDS <= 60)) ||
        die "Media manual cleanup polling must be between 1 and 60 seconds."
    ((KURASTORAGE_MEDIA_CLEANUP_RUN_LEASE_MINUTES >= 1 && KURASTORAGE_MEDIA_CLEANUP_RUN_LEASE_MINUTES <= 1440)) ||
        die "Media cleanup run lease must be between 1 and 1440 minutes."
