...
- 外部索引再スキャンはDomainの`Indexing/IndexScanRun.cs`・`IndexScanItem.cs`、Applicationの`Indexing/IndexScanService.cs`・`IndexingContracts.cs`・`Abstractions/IndexingAbstractions.cs`、Infrastructureの`Persistence/IndexCatalogRepository.cs`・`Storage/ManagedFileSystemSnapshotReader.cs`・`Indexing/IndexScanLogObserver.cs`、Admin CLIの`Program.cs`に配置する。
