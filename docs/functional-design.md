...
- 写真・PDF Viewerは参考UIのヘッダー、表示領域、主要操作の階層を維持し、横画面または文字拡大時は主要操作を隠さず縦スクロールで到達可能にする。
...
- Android 13以降で通知権限がない場合は大量転送を開始せず、永続Queueを保持したまま通知権限待ちとして終了する。
...
- File・Folderの名前変更Dialog、移動先Picker、処理中、成功、主要Error表示
- 移動先Pickerで対象Folder、配下、現在親、Trash項目を確定できないこと
- 名前変更・移動の成功後と通信結果不明時に一覧・詳細をServerから再取得すること
...
8. LANとZeroTierの両経路でFile・Folderを名前変更し、Fileと配下を持つFolderを移動する。変更前後でFile ID、`fileVersion`、内容SHA-256が一致する。
9. 同名Rename・Move、Folderの自分自身・子孫への移動、Root・Trash・他User項目の変更を拒否し、既存項目を上書きしない。
10. 他UserのFile IDを指定した一覧外操作が拒否され、存在情報を過度に公開しない。
11. HDDを未マウントにしてUpload・名前変更・移動を拒否し、OSルートへ誤保存しない。途中状態は再起動後に復旧または`RECOVERY_REQUIRED`となる。
12. AndroidとRaspberry Piを再起動し、接続確認、再ログイン、主要操作を再度完了する。
...
- ダウンロードと保存先選択
- フォルダ作成
- 通常一覧または詳細から開く名前変更Dialog。現在名を初期表示し、基本的な入力検証後に実行する
- 個人Rootから開始する移動先Picker。Folder階層の移動、戻る、移動先確認を提供し、対象Folderと配下および現在親を確定不可にする
- ゴミ箱移動
- 復元
- 操作中の進捗、キャンセル、失敗後再試行
- 同名競合では既存ファイルを上書きせず、競合を表示して操作を終了する
- 通信結果不明、移動先消失、Storage異常、Recovery要求では成功状態をローカル合成せず、Serverから一覧と詳細を再取得する
