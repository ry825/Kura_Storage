...
Session作成のIdempotency KeyはNetwork要求より先にRoomへ永続化する。応答不明またはProcess中断後は新しいKeyやSessionを発行せず、同じKeyでSession作成を再送してServerの既存Sessionを再取得する。Session ID保存後はSession GETとServer確定offsetから再開し、WorkManagerのJob自体が失われてもRoomの未完了項目を正として別Jobが完了へ収束させる。

各Transfer Workerはclaim前に現在Account Scopeの期限切れleaseを回収する。Session IDがない項目は通常のPENDING、Session IDがある項目は`SERVER_RECONCILIATION`付きPENDINGへ戻し、後者はSession GETでServer確定offsetを照合してから再開する。手動の「今すぐバックアップ」は待機中の指数backoff chainを置き換えて即時Scan／Transferを予約する。取消・通常終了時はWorker自身がclaimした未完了leaseを`NonCancellable`な終了処理で即時解放し、次のWorkerが待ち時間なく再claimする。Process強制終了で終了処理を実行できない場合は、期限切れlease回収をフォールバックとして同じ永続Queueへ収束させる。
