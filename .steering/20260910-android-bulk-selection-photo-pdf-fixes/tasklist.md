...
> 実装開始時の確認（2026-09-10）: 報告対象の一括選択UIは、現在の`main`ではなく未Mergeの先行branch `fix/android-backup-media-reliability` に存在する。先行PRのmainマージ後、その最新`main`を基点にPR 1を開始する。先行変更を重複実装しないため、この時点では実装を開始しない。

## フェーズ0: 実装開始・安全な事前確認

- [ ] PR 1の実装前状態を確定する。
  - [ ] `.steering/20260910-android-bulk-selection-photo-pdf-fixes/requirements.md`、`design.md`、本ファイルと関連する正式文書の写真Viewer、PDF、Android UI/テスト節を再確認する。
  - [ ] `git status`、`git diff --check`、現在Branch、先行Pull Request/CI状態を確認し、既存の未Commit変更を対象外として記録する。
  - [ ] 先行変更が未Mergeの場合はその依存を解消してから、最新`main`を基点とするPR 1用短命Branchを準備する。既存変更を含むBranchで安全に分離できない場合は実装を開始せず報告する。
  - [ ] `FileBrowserScreen`のselection/bulk-trash、`PhotoViewerScreen`/PhotoCanvas、PDF route/`PdfViewerViewModel`/`TemporaryPdfStore`/`PdfDocumentController`、対応Testの現行構造を確認する。
...
- [ ] 写真全画面の視認性とSwipe遷移を最小fixtureで再現する。
  ...
  - [ ] Swipe時に`fullscreen`が解除される箇所と、Photo ID/source変更時に全画面stateが再初期化される箇所を確定する。
...
- [ ] 選択actionの表示コンテナを修正する。
  - [ ] `FlowRow`または幅に応じたRow/Columnによって、幅不足時にactionを安全に折り返す。
  - [ ] actionごとに48dp以上のtouch target、目的が分かるcontent description、状態を色だけに依存しない表示を適用する。
  - [ ] 既存のselection ID、confirmation dialog、bulk-trash use case、認可、error handlingを変更していないことを確認する。

- [ ] 選択actionの対象検証を完了する。
  - [ ] `:feature-files:testDebugUnitTest`と対象Compose/Instrumented testを実行する。
  - [ ] 実機で1件・複数件の選択、Clear、Trash確認、進行中表示を確認する。

## フェーズ3: 写真全画面controlsとSwipeの修正

- [ ] 全画面写真controlsとSwipeの失敗テストを追加する。
  - [ ] 全画面時のbottom controlsがsafe inset、背景surface、48dp target、content descriptionを持つCompose/Instrumented testを追加する。
  - [ ] 等倍の左右Swipeが1 gestureにつき1回だけ前/次callbackを呼び、全画面stateを解除しないtestを追加する。
  - [ ] zoom中pan、先頭/末尾、loading/error、連続Swipe、旧image responseの破棄をUnit/Compose testで固定する。

- [ ] 全画面写真controlsの視認性を修正する。
  - [ ] bottom controlsを写真内容と分離したcontrast surface/scrimへ配置し、safe drawing/navigation bar insetを避ける。
  - [ ] 狭幅と文字拡大でactionが切れないよう折返しまたはadaptive layoutを適用する。
  - [ ] Backは全画面だけを閉じ、操作ボタンは全画面中も安定して操作できるようにする。

- [ ] 全画面を維持するSwipe遷移を実装する。
  - [ ] PhotoCanvasのgesture arbitrationを等倍・水平・閾値超過時だけの1回navigationに限定する。
  - [ ] gesture発火後の重複navigationを抑止し、zoom pan、垂直gesture、edge、前後なしをno-opにする。
  - [ ] Photo ID/sourceの更新で`fullscreen`を意図せずfalseへ戻さず、既存ViewModelの順序・request generation・prefetchを利用する。

- [ ] 写真Viewerの対象検証を完了する。
  - [ ] `:feature-media:testDebugUnitTest`、対象Compose/Instrumented test、必要な`:app:testDebugUnitTest`を実行する。
...
- [ ] 原因確定済みPDF失敗境界のテストを先に追加する。
  ...
  - [ ] renderer open/render/retry/disposeでdescriptor、renderer、page、leaseがcloseされるtestを追加する。
...
- [ ] PDFの失敗原因を最小変更で修正する。
  ...
  - [ ] retry開始前に旧load/render job、旧document、old leaseを安全にclose/cancelし、古い結果がretry stateを上書きしないようにする。
  - [ ] `PdfRenderer`には完全なscope内private fileだけを渡し、256MiB、512MiB、64KiB streaming、空き容量、signature、TTL、logout/Session cleanupの契約を維持する。
  - [ ] 新規PDF library、外部viewer、恒久保存、認可のclient-side迂回を追加しない。

- [ ] PDF表示の対象検証を完了する。
  - [ ] `:core-data:testDebugUnitTest`、`:feature-media:testDebugUnitTest`、対象PDF Instrumented testを実行する。
...
- [ ] 変更対象を高速に統合検証する。
  - [ ] `./apps/android/gradlew -p apps/android :feature-files:testDebugUnitTest :feature-media:testDebugUnitTest :core-data:testDebugUnitTest`を実行する。
  - [ ] `./apps/android/gradlew -p apps/android :feature-files:ktlintCheck :feature-files:detekt :feature-files:lintDebug :feature-media:ktlintCheck :feature-media:detekt :feature-media:lintDebug :core-data:ktlintCheck :core-data:detekt :core-data:lintDebug`を実行する。
  - [ ] `./apps/android/gradlew -p apps/android :app:assembleDebug :feature-files:assembleDebugAndroidTest :feature-media:assembleDebugAndroidTest`を実行する。
  - [ ] 利用可能なAndroid実機またはemulatorで`:feature-files:connectedDebugAndroidTest :feature-media:connectedDebugAndroidTest`を`--max-workers=1`で実行する。

- [ ] 最終のrepository標準Android検証を完了する。
  - [ ] `./scripts/ci/verify-android.sh`を実行し、build、JVM tests、coverage、ktlint、detekt、Android lint、APK/SBOM生成が成功することを確認する。
  - [ ] `git diff --check`と対象差分のself-reviewを実行し、debug code、機密情報、無関係な変更、generated artifactが含まれないことを確認する。

- [ ] 今回追加した検証資材だけを安全に清掃する。
  - [ ] manifestの全entryについてexact ID/path、run ID、現在のidentityを再読込し、baseline対象でないことを確認する。
  - [ ] manifestで確認できたUser、File、Folder、Tag、Favorite、Share、Recent、Activity、Backup、Media job/派生データ、端末temporary fileだけを個別に削除する。
  - [ ] cleanup guardが拒否した対象は削除せず、理由を記録して報告する。
  - [ ] manifest対象が0件になったこと、baselineの既存ID/件数/checksumが不変であることをread-onlyで再確認する。

## フェーズ6: ドキュメント・Pull Request・完了記録

- [ ] 正式文書への更新要否を確認する。
  - [ ] 実装が既存の正式仕様・設計の範囲内なら、更新不要の理由を記録する。
  - [ ] 仕様・設計を変える必要が判明した場合は、`docs/product-requirements.md`、`functional-design.md`、`architecture-design.md`、`repository-structure.md`、`development-guidelines.md`の該当箇所を同じPull Requestで整合更新する。
...
- [ ] 英語のPR 1を1本作成する。
  - [ ] PR titleとbodyにPurpose、Scope、Changes、Tests、Impact/Not performedを英語で記載する。
  - [ ] baseを`main`とし、Mergeは行わない。
